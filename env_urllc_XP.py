...
class ENV_paper(gym.Env):
    def __init__(self, lambda_rate, D_max, xi, max_power, snr_feedback, harq_type):
        ...
        state_dim = 7 if snr_feedback else 6
    ...
    def reset(self, seed=None):
        ...
        self.previous_snrs = []
        self.previous_rates = []
        state = [self.q_t, self.A_t, self.d_t, self.k, self.gamma_k, self.R_sum, self.I_sum]
    ...
    def step(self, action):
        ...
        if self.harq_type == 'CC':
            ...
            success = (self.I_sum >= target_R) & (np.log2(1 + snr) >= R_t)
        ...
        if self.harq_type == 'XP':
            state.extend([self.R_sum, self.I_sum])
        ...
        info = {
            ...
            'R_sum': self.R_sum,
            'I_sum': self.I_sum,
            'bits_successful': S_t # So bit truyen thanh cong
        }
